...
# --- Define class labels ---
# IMPORTANT: Modified to match the saved model which has 3 classes!
...
def generate_gradcam(model, input_tensor, class_idx):
    ...
    try:
        ...
        st.error(f"Error during Grad-CAM calculation: {e}")
    finally:
        # Clean up
    ...
    if uploaded_file:
        try:
            ...
            if model is not None:
                ...
                st.error("Model could not be loaded. Cannot generate Grad-CAM visualization.")
        except Exception as e:
